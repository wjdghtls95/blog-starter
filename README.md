...
| 2 | LLM API key (pick one — see below) | See LLM options section | 2 min |
...
The quiz is generated by whichever LLM you configure. Set `LLM_PROVIDER` in GitHub Secrets to pick one:

| Provider | Secret to set | `LLM_PROVIDER` value |
|----------|--------------|----------------------|
| OpenAI | `OPENAI_API_KEY` | `openai` (default if not set) |
| Groq | `GROQ_API_KEY` | `groq` |
| Anthropic (Claude) | `ANTHROPIC_API_KEY` | `anthropic` |
| Ollama / OpenRouter / Together / etc. | `LLM_API_KEY` + `LLM_BASE_URL` + `LLM_MODEL` | `openai-compatible` |

You can also set `LLM_MODEL` to override the default model for your chosen provider.
...
**LLM (set only the one matching your provider):**

| Secret | When to set |
|--------|-------------|
| `LLM_PROVIDER` | Set to `groq`, `anthropic`, or `openai-compatible` — omit to use OpenAI |
| `GROQ_API_KEY` | Groq (free tier) |
| `OPENAI_API_KEY` | OpenAI (default if `LLM_PROVIDER` not set) |
| `ANTHROPIC_API_KEY` | Anthropic / Claude |
| `LLM_API_KEY` + `LLM_BASE_URL` + `LLM_MODEL` | Self-hosted or OpenRouter |
